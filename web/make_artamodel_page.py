...
<p>Nothing leaves the page: the ephemeris, the three geography boosters and the ArtaModel stages load into Pyodide and are evaluated here. Births are cast at 09:00 local mean time at the birthplace (the fit used the zone clock; for the outer planets the model reads, the difference is under 0.01°). Dates may be day (<code>1936-08-04</code>), month (<code>1936-08-00</code>) or year (<code>1936-00-00</code>) precision — the start too: <code>1968-00-00</code> is "the year 1968", and 1 January is an ordinary day.</p>
...
<p>For a pair, every day from a date (today by default) over a horizon is scored — the start day enters through the two ages, its weekday and month, and the outer planets of that day against each natal chart — and the twenty best <b>options</b> are listed: the best day of each month, ranked, so twenty consecutive Tuesdays count as one option, not twenty. Every calendar day is scored; "year only" is spelled <code>YYYY-00-00</code>. What the model prefers is, again, what it learned: later starts (older ages), certain weekdays, and the slow sky of the day.</p>
...
<fieldset><legend>The horizon</legend><label>From (default today) <input name=bfrom value="" placeholder="YYYY-MM-DD"></label><label>Years ahead <input name=byears value="5"></label><label>How many days <input name=btop value="20"></label><button id=bestgo type=button disabled>Loading…</button></fieldset>
</form>
<div id=bresult class=result hidden><div id=bsummary class=detail></div><div id=btable class=detail></div></div>
...
P._CAPS = json.load(open("/capitals.json"))`);
    status("ready — the model runs in this page"); $("#go").disabled = false; $("#go").textContent = "Predict"; $("#find").disabled = false; $("#find").textContent = "Find the top matches"; $("#bestgo").disabled = false; $("#bestgo").textContent = "Find the best days"; run();
...
async function bestday() {{
  if (!py) return; const f = $("#bestday"); const v = n => f.elements[n].value.trim(); $("#bestgo").disabled = true; $("#bestgo").textContent = "Searching…"; status("scoring every day of the horizon…");
  try {{
    const t0 = performance.now();
    const out = await py.runPythonAsync(`import json, stack_iv_predictor as P
r = P.best_start_days(${{JSON.stringify(v("bdob1"))}}, float(${{JSON.stringify(v("blat1"))}}), float(${{JSON.stringify(v("blon1"))}}), ${{JSON.stringify(v("bdob2"))}}, float(${{JSON.stringify(v("blat2"))}}), float(${{JSON.stringify(v("blon2"))}}), from_date=(${{JSON.stringify(v("bfrom"))}} or None), years=int(${{JSON.stringify(v("byears") || "5")}}), top=int(${{JSON.stringify(v("btop") || "20")}}))
json.dumps(r)`);
...
$("#bestgo").addEventListener("click", bestday);
{{ const f = $("#bestday"); if (!f.elements.bfrom.value) f.elements.bfrom.value = new Date().toISOString().slice(0, 10); }}
{{ const f = $("#finder"); if (!f.elements.fstart.value) f.elements.fstart.value = new Date().toISOString().slice(0, 10); }}   // the start date defaults to TODAY, shown
