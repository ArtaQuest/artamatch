...
BAGS = int(os.environ.get("AQ_BAGS", "5"))                  # half-samples per training fold
BAG_MIN = int(os.environ.get("AQ_BAG_MIN", "3"))             # a phasor is STABLE if chosen in >= this many bags
BAG_FRAC = float(os.environ.get("AQ_BAG_FRAC", "0.5"))       # share of training COMPONENTS in a half-sample
BAG_BASE = os.environ.get("AQ_BAG_BASE", "1") == "1"         # also run the plain stepwise on the same folds (same-fold baseline)
TAG = "comp_bagging_" + TAG + f"_b{BAGS}m{BAG_MIN}"
ABLATE = True     # this copy is an outer estimate only: the deploy pass below is the UNBAGGED procedure and must never ship under this tag
...
# ---- the nested estimate, BAGGED SELECTION (competition lens "bagging") ------------------------
# Inside each outer training fold the stepwise runs on BAGS half-samples of the training
# COMPONENTS (stability selection, Meinshausen & Buehlmann 2010: subsample without replacement,
# by family so no couple's relatives straddle a bag). Four models are scored on the untouched
# outer fold, all from the same bag orderings, nothing chosen on the test fold:
#   stable  : phasors chosen in >= BAG_MIN bags, ranked (count desc, mean position asc), capped
#             at KMAX, fast bodies forced on the FULL training fold, refit on the full training
#             fold.                                                         <- PRIMARY
#   ens     : each bag's selection (+forced) refit on the FULL training fold; outer logits averaged.
#   ensbag  : each bag's selection (+forced) refit on ITS OWN bag rows; outer logits averaged
#             (true bagging).
#   base    : the plain stepwise on the full training fold (the standing procedure) — the
#             same-fold baseline, so the comparison does not depend on a different fold split.
def refit_scores(sel, rows):
    wm_t = torch.from_numpy((w * rows).astype(np.float32)).to(DEV)
    Amat = design(sel)
    beta = newton_on(Amat, wm_t, RL, ksel=[MET[c]["k"] for c in sel])
    v = (Amat @ torch.from_numpy(beta.astype(np.float32)).to(DEV)).cpu().numpy()
    del Amat
...
VARIANTS = ["stable", "ens", "ensbag"] + (["base"] if BAG_BASE else [])
OOF = {v: np.zeros(n, np.float32) for v in VARIANTS}
oof_outer = OOF["stable"]
per_fold = []
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    orders, bag_masks = [], []
    for b in range(BAGS):
        half = np.random.default_rng(5000 + 100 * kf + b).random(gid.max() + 1)[gid] < BAG_FRAC
        bag = trm & half
        bag_masks.append(bag)
        orders.append(stepwise(bag, KMAX, seed=1000 + kf + 37 * b))
        log(f"      fold {kf+1} bag {b+1}/{BAGS}: {bag.sum():,} rows · {len(orders[-1])} phasors · {time.time()-tf:.0f}s")
    cnt = Counter(c for o in orders for c in o)
    pos = {c: float(np.mean([o.index(c) for o in orders if c in o])) for c in cnt}
    stable_all = sorted([c for c, k in cnt.items() if k >= BAG_MIN], key=lambda c: (-cnt[c], pos[c]))
    stable = stable_all[:KMAX]
    sel_s, forced_s = force_fast(stable, trm)
    v = refit_scores(sel_s, trm); OOF["stable"][~trm] = v[~trm]
    # ensembles
    ens_full = np.zeros(n, np.float32); ens_bag = np.zeros(n, np.float32)
    for b in range(BAGS):
        sel_b, _ = force_fast(orders[b], trm)
        ens_full += refit_scores(sel_b, trm) / BAGS
        sel_bb, _ = force_fast(orders[b], bag_masks[b])
        ens_bag += refit_scores(sel_bb, bag_masks[b]) / BAGS
    OOF["ens"][~trm] = ens_full[~trm]; OOF["ensbag"][~trm] = ens_bag[~trm]
    rec = {"n_union": len(cnt), "n_stable": len(stable_all), "K_stable": len(stable),
           "n_forced": len(forced_s), "forced": [b for b, _ in forced_s],
           "count_hist": {str(k): sum(1 for c in cnt if cnt[c] == k) for k in range(1, BAGS + 1)},
           "fold_auc": {}}
    if BAG_BASE:
        order0 = stepwise(trm, KMAX, seed=1000 + kf)
        sel0, forced0 = force_fast(order0, trm)
        v0 = refit_scores(sel0, trm); OOF["base"][~trm] = v0[~trm]
        rec["n_forced_base"] = len(forced0)
        rec["stable_vs_base_overlap"] = len(set(stable) & set(order0))
    for vn in VARIANTS:
        rec["fold_auc"][vn] = round(float(roc_auc_score(y[~trm], OOF[vn][~trm])), 4)
    per_fold.append(rec)
    if DEV == "mps": torch.mps.empty_cache()
    log(f"   outer {kf+1}/{NOUTER} · union {len(cnt)} · stable(>={BAG_MIN}/{BAGS}) {len(stable_all)} -> K {len(stable)} "
        f"(+{len(forced_s)} forced: {','.join(b for b,_ in forced_s) or 'none'}) · fold AUC "
        + " · ".join(f"{vn} {rec['fold_auc'][vn]:.4f}" for vn in VARIANTS) + f" · {time.time()-tf:.0f}s")
...
def _within_era(o):
    num = den = 0.0
    for d in np.unique(_dec[_m]):
        r = _m & (_dec == d)
        if r.sum() >= 200 and 0 < y[r].sum() < r.sum():
            num += roc_auc_score(y[r], o[r]) * r.sum(); den += r.sum()
    return num / den if den else float("nan")
VAR_RES = {}
for vn in VARIANTS:
    a_n = float(roc_auc_score(y[_m], OOF[vn][_m])); a_w = _within_era(OOF[vn])
    VAR_RES[vn] = {"nested_auc": round(a_n, 4), "within_era_auc": round(a_w, 4)}
    log(f"VARIANT {vn:<7} NESTED {a_n:.4f}  WITHIN-ERA {a_w:.4f}")
    np.save(f"{D_}/oof_nested_{TAG}_{vn}.npy", OOF[vn])
...
if ABLATE:
    json.dump({"tag": TAG, "nested_auc": round(auc_nested, 4), "within_era_auc": round(auc_within_era, 4), "per_fold": per_fold,
               ...
               "variants": VAR_RES, "bags": BAGS, "bag_min": BAG_MIN, "bag_frac": BAG_FRAC},
