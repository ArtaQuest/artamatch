"""comp_harmonic-mapping.py — COMPETITION COPY of fit_nested.py (never edit the original).
Changes from fit_nested.py, all marked HARMONIC-MAPPING:
  (1) TAG prefixed "comp_harmonic-mapping_" so no output collides;
  (2) AQ_SYSTEMS=1 loads AQ_SYSFILES (comma list, default comp_harmonic-mapping_systems.npz written
      by build_harmonic_mapping.py; "systems.npz,comp_harmonic-mapping_systems.npz" adds the naive
      twenty as well) — the traditions' OWN state-to-angle placements (Lo Shu compass, Tzolkin
      colour, Chaldean week, nayin/branch elements, branch seasons, manzil quarters) as pseudo-bodies;
  (3) score_all: a candidate angle whose sine column is identically zero (only possible on 2-state
      circles; none here, kept for safety) falls back to the exact 1-df score gc^2/Scc;
  (4) each outer fold logs which mapped-body terms entered.
...
TAG = "comp_harmonic-mapping_" + TAG          # HARMONIC-MAPPING: never collide with shared outputs
SYSFILES = [x for x in os.environ.get("AQ_SYSFILES", "comp_harmonic-mapping_systems.npz").split(",") if x]   # HARMONIC-MAPPING
...
if SYSTEMS:
    ...
    for _sf in SYSFILES:                                     # HARMONIC-MAPPING: one or more systems files
        SZ = np.load(f"{D_}/{_sf}", allow_pickle=True)
        log(f"systems file {_sf}: {list(map(str, SZ['names']))}")
    ...
    assert len(bod) == len(set(bod)), "duplicate body names across systems files"
...
def score_all(r, vw, taken, mask=None, X=None):
    ...
    for lo in range(0, p, 1024):
        ...
        # HARMONIC-MAPPING: a 2-state circle would give sin == 0 exactly (0/0 in the 2-df ratio);
        # exact 1-df score test on the cosine alone there. No such column in this bank; kept for safety.
        degenerate = Sss <= 1e-6 * Scc
        zz = torch.where(degenerate, gc * gc / (Scc + eps), zz)
        rho2 = torch.where(degenerate, torch.zeros_like(zz), (Scs * Scs) / (Scc * Sss + eps))
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    # HARMONIC-MAPPING: name what entered, so the report can say which mapped circles earned a place
    _sysn = [MET[c]["label"] for c in sel if NSTATES[MET[c]["i"]] or NSTATES[MET[c]["j"]]]
    log(f"      mapped-body terms in fold {kf+1}: {len(_sysn)} of {len(sel)} · " + " | ".join(_sysn))
    per_fold[-1]["mapped_terms"] = _sysn
