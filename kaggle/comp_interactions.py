"""comp_interactions.py — COMPETITION COPY of fit_nested.py (lens "interactions"). Never edit the
original; this copy differs ONLY in the bank: SECOND-ORDER PHASORS. A product of two k=1 phasors
cos(u)cos(v) is a sinusoid of the combined angles u+v and u-v, so the only honest way to admit
"interactions" into an additive phasor model is as phasors of SUM/DIFFERENCE angles. The whole
4-body space lost under nesting (too many duds), so here the candidates are restricted to the
sums and differences of the FIRST AQ_INTER_TOP (default 10) phasors the lean stepwise picks
INSIDE THE TRAINING FOLD — the fold's own "top-10", never the deployed model's (that would be
chosen on the test fold). With AQ_INTER_TOP=0 the script is exactly fit_nested.py (the control).
...
INTER_TOP = int(os.environ.get("AQ_INTER_TOP", "10"))       # phasors whose pairwise sums/diffs become candidates (0 = control)
TAG = "comp_interactions_" + TAG + f"_top{INTER_TOP}"
assert os.environ.get("AQ_GROUP", "0") != "1", "comp_interactions: GROUP selection not supported"
...
def _coef(kind, i, j):
    """integer coefficient vector of a 2-body angle over (his bodies, her bodies) — the algebra
    the interaction builder uses to recognise a sum/difference that collapses to an angle it
    already holds (e.g. (his A - her B) - (his C - her B) = his A - his C)."""
    c = np.zeros(2 * NB, np.int8)
    if kind == "xdiff":  c[i] += 1; c[NB + j] -= 1
    elif kind == "aspM": c[i] += 1; c[j] -= 1
    elif kind == "aspW": c[NB + i] += 1; c[NB + j] -= 1
    else: raise ValueError(kind)          # lean bank only (XY/XX/YY); sums/DD are not combined here
    return c
def _canon(c):
    nz = np.flatnonzero(c)
    if len(nz) == 0: return None
    if c[nz[0]] < 0: c = -c               # negating an angle only flips the sine's sign: same 2-df phasor
    return tuple(int(x) for x in c)
BASE_ANG = [(a[0], a[1], a[2], a[3], a[4], a[5], frozenset({bod[a[3]], bod[a[4]]}), _coef(a[2], a[3], a[4])) for a in ANG]
BASE_KEYS = {_canon(a[7]) for a in BASE_ANG}
MET = []; THETA = None; p = 0; A_IDX = None; K_VAL = None; INV = {}; BASE_P = 0
def build_bank(extra):
    """(re)build the candidate bank = the lean base + `extra` interaction angles. Base phasors keep
    their indices (extras are appended), so a selection made on the base stays valid."""
    global ANG, MET, THETA, p, A_IDX, K_VAL, INV, BASE_P
    ANG = BASE_ANG + list(extra)
    MET = []
    for a, (ang, name, kind, i, j, fam, bodies, coef) in enumerate(ANG):
        for k in HARM:
            if any(NSTATES[x] and k % NSTATES[x] == 0 for x in (i, j)):
                continue        # constant on a discrete circle
            MET.append({"a": a, "k": k, "kind": kind, "i": i, "j": j, "fam": fam, "bodies": bodies,
                        "angle_name": name,
                        "label": f"{k}*({name})" if k > 1 else name})
    if not extra: BASE_P = len(MET)
    ...
    INV = {b: torch.tensor([b in m["bodies"] for m in MET], device=DEV) for b in FAST}
    if DEV == "mps": torch.mps.empty_cache()
def interactions_of(sel):
    """every SUM and DIFFERENCE of the distinct angles behind the phasors in `sel` (a training-fold
    selection), minus the ones that collapse to nothing or to an angle already in the bank."""
    seen = set(BASE_KEYS); out = []
    angs = []
    for c in sel:
        a = MET[c]["a"]
        if a < len(BASE_ANG) and a not in angs: angs.append(a)
    for u, v in itertools.combinations(angs, 2):
        au, av = BASE_ANG[u], BASE_ANG[v]
        for sign, sym in ((1, "+"), (-1, "-")):
            key = _canon(au[7] + sign * av[7])
            if key is None or key in seen: continue
            seen.add(key)
            th = np.mod(au[0] + sign * av[0] + np.pi, 2 * np.pi) - np.pi
            out.append((th, f"({au[1]}) {sym} ({av[1]})", "int" + sym, au[3], au[4], "INT",
                        au[6] | av[6], au[7] + sign * av[7]))
    return out
build_bank([])
log(f"{len(ANG)} angles x {len(HARM)} harmonics = {p:,} phasors · {n:,} couples · KMAX {KMAX} · INTER_TOP {INTER_TOP}")
...
GSIZE = len(HARM)
INV_ANG = {}   # GROUP selection is refused above
...
def stepwise(trm_np, kmax, seed=0):
    ...
    cur_auc = 0.5
    build_bank([])                                   # every selection starts from the lean base
    ...
    sel = []; angles = []
    for k in range(kmax):
        if INTER_TOP and k == INTER_TOP and len(sel) == INTER_TOP:
            # SECOND-ORDER CANDIDATES: the first INTER_TOP picks of THIS training fold define the
            # basis; their pairwise sums/differences join the bank. The picks so far were made and
            # (with AQ_VALIDATE) validated exactly as in the control, so the only difference from
            # here on is the wider bank — which the same ortho score test + validation must earn.
            extra = interactions_of(sel)
            build_bank(extra)
            log(f"      interactions: {len(extra)} sum/diff angles from the first {INTER_TOP} picks → bank {p:,} phasors")
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    n_int = sum(1 for c in sel if c >= BASE_P)
    per_fold.append({"K_inner": Kin, "n_forced": len(forced), "fold_auc": round(fauc, 4),
                     "forced": [b for b, _ in forced], "n_inter": n_int,
                     "inter_terms": [MET[c]["label"] for c in sel if c >= BASE_P]})
    log(f"   outer {kf+1}/{NOUTER} · K_inner {Kin} (+{len(forced)} forced: {','.join(b for b,_ in forced) or 'none'}) · {n_int} interaction term(s) · fold AUC {fauc:.4f}")
    for c in sel:
        if c >= BASE_P: log(f"      INT  {MET[c]['label']}")
...
if ABLATE:
    json.dump({"tag": TAG, "nested_auc": round(auc_nested, 4), "within_era_auc": round(auc_within_era, 4), "per_fold": per_fold,
               ...
               "inter_top": INTER_TOP, "base_phasors": BASE_P},
