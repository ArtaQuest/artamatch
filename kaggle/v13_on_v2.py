"""v13_on_v2.py — the declared v13 recipe re-run unchanged on corpus v2 (collapsed-duplicate label,
+50k recovered mono-married negatives). A new corpus is a new series: declare best-by-CV, read test once.
Constraint, bank, ladder identical to v13_fit."""
...
def main():
    ...
    print(f"  corpus v2: train {len(tr):,} ({yi.mean():.1%}) · test {len(te):,} ({yte.mean():.1%})", flush=True)
    print(f"  bank {len(names):,} · both-date singles {len(both_ix):,}", flush=True)
    ...
    # the label rate halved (2.4%), so the alpha scale shifts down — bracket it
    ...
    print(f"\n  v13-recipe ON CORPUS V2 · stage {stage} alpha={alpha} · {len(weights)} rules "
    ...
    json.dump({"model": "ArtaMatch v13 recipe on corpus v2 (pair-only)", "alpha": alpha, "stage": stage,
              ...
              open(os.path.expanduser("~/.artamatch-dev/v13v2_model.json"), "w"), indent=1)
    np.save(os.path.expanduser("~/.artamatch-dev/v13v2_test_z.npy"), zt)
    print("  saved v13v2_model.json + v13v2_test_z.npy")
