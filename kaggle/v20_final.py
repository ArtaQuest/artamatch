...
def main():
    ...
    XW, nW = F19.families19(tr, Z, "train"); XWt, _ = F19.families19(te, Z, "test")
    supW = XW.sum(0) >= 40
    XW, XWt = XW[:, supW], XWt[:, supW]; nW = [n for n, s_ in zip(nW, supW) if s_]
    ...
    which = os.environ.get("AQ_BANK", "base")
    if which == "wave4":
        Xall = np.column_stack([Xall, XW]); Xallt = np.column_stack([Xallt, XWt])
        names = names + nW
    del XW, XWt
    dep = np.array([side(n) for n in names])
    both_ix = np.where(dep == "AB")[0]
    print(f"  DOCTRINE-ONLY bank [{which}] {len(names):,} ({int((~keep).sum())} calendar features removed) · "
          f"both-date {len(both_ix):,} · train {len(tr):,} ({yi.mean():.1%}) test {len(te):,} ({yte.mean():.1%})", flush=True)
    ...
    json.dump({"bank": which, "stage": stage, "alpha": alpha, "cv": cv},
              open(os.path.expanduser(f"~/.artamatch-dev/v20_cv_{which}.json"), "w"))
    if not os.environ.get("AQ_READ"):
        print(f"  CV declared for bank [{which}]: stage {stage} alpha={alpha} CV {cv:.4f} — no read (race mode)")
        return
    ...
    print(f"\n  v20 FINAL stage {stage} alpha={alpha} · {len(weights)} rules "
