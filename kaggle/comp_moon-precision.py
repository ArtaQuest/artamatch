"""comp_moon-precision.py — COPY of fit_nested.py for the "moon-precision" competition entry.

THE LENS: THE MOON AND TIME. ~/.artamatch-dev/birthtime.csv does not exist (checked 2026-09-02), so
this is the fallback the lens prescribes: a Moon-UNCERTAINTY-AWARE bank. Every chart is cast at
NOON UT with the birth hour unknown, so the Moon (~13.2 deg/day) carries a uniform +-6.6 deg blur.
A harmonic k of any Moon angle is attenuated by sinc(k*6.6deg): k=1 0.998, k=2 0.991, k=10 0.79,
k=27 0.07, k=36 ~0.03. The all-harmonic ortho run (report_nested_k32_ortho.json, 0.6911) selected
36*(his moon - his mars) and 7*(her moon - her mercury) — the first is physically unresolvable
at noon and can only be an in-sample lottery win. AQ_MOON_KMAX (default 2, per the lens) removes
every Moon-involving phasor above that harmonic from the candidate bank BEFORE selection; nothing
else changes. Bodies other than the Moon move <= 2 deg/day (blur <= +-1 deg, sinc(36*1deg)=0.94),
so their ladder is untouched. This is a NARROWING of the all-harmonic XY bank, tested against the
uncapped all-harmonic XY bank and the standing k=1-only XY bank under identical settings.
...
MOON_KMAX = int(os.environ.get("AQ_MOON_KMAX", "2"))            # moon-precision: drop Moon phasors with k > this (0 = no cap)
...
TAG = "comp_moon-precision_" + TAG + (f"_moonk{MOON_KMAX}" if MOON_KMAX else "_moonuncapped")
...
for a, (ang, name, kind, i, j, fam) in enumerate(ANG):
    for k in HARM:
        ...
        if MOON_KMAX and k > MOON_KMAX and "moon" in (bod[i], bod[j]):
            continue        # moon-precision: noon blur +-6.6 deg makes this harmonic unresolvable
...
_nmoon = sum(1 for m in MET if "moon" in (bod[m["i"]], bod[m["j"]]))
log(f"MOON-PRECISION: Moon phasors capped at k<={MOON_KMAX or 'none'} · {_nmoon:,} Moon phasors remain in the bank")
