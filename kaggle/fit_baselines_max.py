...
def cv(cols, label):
    ...
    del Ft
    print(f"  {label:<34}{F.shape[1]:>5} params   AUC {best[0]:.4f}  @rl {best[1]:g}", flush=True)
    return best[0]
...
json.dump({"him_only": a_him, "her_only": a_her, "shipped_families": a_pair, "age_gap": a_gap,
           "lift_over_best_solo": a_pair - max(a_him, a_her)},
          open(f"{D_}/report_baselines_max.json", "w"), indent=1)
print(f"\nsaved report_baselines_max.json")
