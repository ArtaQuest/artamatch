"""comp_names-variants.py — COMPETITION COPY of fit_nested.py (lens "names-variants").
Adds AQ_NAMES=1: name-numerology variants from AQ_DIR/systems_names.npz (build_names.py) as
pseudo-bodies, exactly the way AQ_SYSTEMS adds systems.npz. Nothing else in the procedure changes.
Outputs are prefixed comp_names-variants_ so they never collide with the shared fitter.
...
NAMES = os.environ.get("AQ_NAMES", "0") == "1"                 # competition: name-numerology pseudo-bodies
TAG = "comp_names-variants_" + TAG + ("_names" if NAMES else "")
...
if NAMES:
    # NAME-NUMEROLOGY VARIANTS AS PSEUDO-BODIES (competition lens): first/surname/full x
    # Pythagorean/Chaldean x expression/soul/personality/hidden passion/master (12-circle).
    # Date-independent by construction; an unlabelled person holds a seeded RANDOM state.
    NZ = np.load(f"{D_}/systems_names.npz", allow_pickle=True)
    N_PLANETS = len(bod)
    bod = bod + [str(x) for x in NZ["names"]]
    NSTATES = NSTATES + [int(x) for x in NZ["nstates"]]
    RA = np.concatenate([RA, np.deg2rad(NZ["theta_a_sys"])], 1)
    RB = np.concatenate([RB, np.deg2rad(NZ["theta_b_sys"])], 1)
    print(f"names: {len(NZ['names'])} pseudo-bodies · labelled him {NZ['has_label_a'].mean():.1%} · her {NZ['has_label_b'].mean():.1%}", flush=True)
else:
    N_PLANETS = len(bod)
...
# FORCING STAYS PLANET-ONLY (competition copy): force_fast is not validated, so a name body may
# enter the model ONLY through the validated inner CV, never by riding on a forced fast body. This
# keeps the forcing step identical between the names arm and the planets-only control.
INV = {b: torch.tensor([(bod[m["i"]] == b or bod[m["j"]] == b) and m["i"] < N_PLANETS and m["j"] < N_PLANETS
                        for m in MET], device=DEV) for b in FAST}
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    name_terms = [MET[c]["label"] for c in sel if MET[c]["i"] >= N_PLANETS or MET[c]["j"] >= N_PLANETS]
    per_fold.append({"K_inner": Kin, "n_forced": len(forced), "fold_auc": round(fauc, 4),
                     "forced": [b for b, _ in forced], "K_selected": len(sel),
                     "terms": [MET[c]["label"] for c in sel], "name_terms": name_terms})
    log(f"      selected {len(sel)} terms · {len(name_terms)} name term(s): {name_terms}")
