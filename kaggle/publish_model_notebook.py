...
SRC = os.path.join(HERE, "model_notebook.py")
STAGE = "/tmp/aqnbpush"
...
SLUG = "artamatch-the-best-astrology-so-far-explained"
TITLE = "ArtaMatch: the best astrology so far, explained"
...
def main():
    ...
    if os.path.exists(STAGE):
        shutil.rmtree(STAGE)
    os.makedirs(STAGE)
    open(os.path.join(STAGE, "model_notebook.ipynb"), "w").write(nb)
    json.dump({
        "id": f"{OWNER}/{SLUG}",
        "title": TITLE,
        "code_file": "model_notebook.ipynb",
        "language": "python",
