...
json.dump({"inner": list(INNER), "n": n, "auc_by_k": aucs, "best": aucs[bk], "best_k": bk,
          ...
          open(f"{D_}/report_inner{len(INNER)}.json", "w"), indent=1)
# THE FILENAME CARRIES THE RUN'S IDENTITY. These two scripts shared report_inner.json and the
# 5-body run silently replaced the 7-body one, which put a wrong number on the live page. The
# exporter reads the named files and asserts they differ.
log(f"saved report_inner{len(INNER)}.json")
