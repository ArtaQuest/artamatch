...
"""
import hashlib, json, os, time, itertools
...
if SYSTEMS:
    ...
    # A TAG IS A FILENAME. Spelling out 150 pseudo-body names blew past the 255-byte limit and the
    # run died AFTER its fit, losing the result; long lists collapse to a count plus a digest.
    _nm = "-".join(SYS_ONLY) if SYS_ONLY else str(len(_sel))
    if len(_nm) > 60: _nm = f"{len(SYS_ONLY)}sel{hashlib.sha1(_nm.encode()).hexdigest()[:8]}"
    TAG = TAG.replace("_systems", "_sys" + _nm)
if ONLY_BODIES:
