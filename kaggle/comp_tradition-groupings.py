"""comp_tradition-groupings.py — COMPETITION COPY of fit_nested.py (never edit the original).
Changes from fit_nested.py, all marked TRADITION-GROUPINGS:
  (1) TAG prefixed "comp_tradition-groupings_" so no output collides;
  (2) AQ_SYSTEMS=1 loads AQ_SYSFILE (default comp_tradition-groupings_systems.npz, written by
      build_tradition_groupings.py) instead of systems.npz — the traditions' compatibility circles
      (trines, secret-friend mirror, stem elements, Kua groups, Nine-Star elements, life-path
      groups) as pseudo-bodies;
  (3) score_all: a candidate angle that lives on {0, 180} (two 2-state circles) has an identically
      zero sine column; the 2-df statistic divides 0 by 0 there and the candidate could never enter.
      It falls back to the exact 1-df score gc^2/Scc. The planet bank has no such column, so the
      standing best is untouched by this.
...
TAG = "comp_tradition-groupings_" + TAG          # TRADITION-GROUPINGS: never collide with shared outputs
SYSFILE = os.environ.get("AQ_SYSFILE", "comp_tradition-groupings_systems.npz")   # TRADITION-GROUPINGS
...
if SYSTEMS:
    ...
    SZ = np.load(f"{D_}/{SYSFILE}", allow_pickle=True)       # TRADITION-GROUPINGS: the groupings file
    log(f"systems file {SYSFILE}: {list(map(str, SZ['names']))}")
...
def score_all(r, vw, taken, mask=None, X=None):
    ...
    for lo in range(0, p, 1024):
        ...
        # TRADITION-GROUPINGS: two 2-state circles give an angle on {0,180}: sin == 0 exactly, so
        # Sss == 0 and the 2-df ratio is 0/0. Exact 1-df score test on the cosine alone there.
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    # TRADITION-GROUPINGS: name what entered, so the report can say which circles earned a place
    _sysn = [MET[c]["label"] for c in sel if NSTATES[MET[c]["i"]] or NSTATES[MET[c]["j"]]]
    log(f"      grouping terms in fold {kf+1}: {len(_sysn)} of {len(sel)} · " + " | ".join(_sysn))
    per_fold[-1]["grouping_terms"] = _sysn
