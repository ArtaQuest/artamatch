"""comp_selection-tuning.py — a COPY of fit_nested.py (never edit the original) for the
"selection-tuning" competition entry. The ONLY change is the STOPPING / PICKING rule of the
forward stepwise; the nested procedure is untouched.

  AQ_ZSTOP=a1,a2,..  multiplicity-corrected stop, evaluated on a GRID of levels from ONE ordering.
                   The exact (orthogonalised) 2-df score statistic of a null candidate is chi2(2),
                   survival exp(-t/2); the maximum over p_free untaken candidates exceeds t with
                   probability ~ p_free*exp(-t/2). A candidate enters only if its statistic beats
                   t* = 2*ln(p_free/alpha) * cal, where cal = sum(w^2 p(1-p))/sum(w p(1-p)) corrects
                   the statistic for the class weights (1.0006 here). alpha=1 is exactly the lens's
                   "expected max of p null scores ~ 2 ln p" rule; smaller alpha is stricter; alpha=0
                   never stops (= the standing K=KMAX procedure, the same-budget CONTROL).
                   Because the pick is still argmax z, the stop only TRUNCATES the training-fold
                   order, so every alpha is read off the same stepwise: per outer fold the order is
                   built once to KMAX with its z-trace, then for each alpha the prefix up to the first
                   failing step is forced (fast bodies) + refit + scored on the untouched outer fold.
                   Identical to stopping live; no inner CV, no shortlist, no VTOL.
                   The threshold RISES with ln p, so a wider bank pays for its width in the test.
  AQ_STAB=1        stability pick: the ortho score is also computed on each of AQ_STAB_NF (5) inner
                   subsamples of the training rows (leave-one-inner-fold-out, group folds) and the
                   candidate entered is the argmax of the MIN over subsamples — the phasor strongest
                   in EVERY subsample, not the in-sample lottery winner. The z-trace / stop test still
                   use the full-training statistic of the entered candidate.
Outputs: ablate_<TAG>.json with one block per alpha (nested, within-era, phasors per fold) and
oof_nested_<TAG>_a<alpha>.npy. With AQ_ABLATE=1 only (no deploy pass in this entry).
...
ZGRID = [float(x) for x in os.environ.get("AQ_ZSTOP", "0").split(",") if x != ""]   # 0 = never stop (control)
STAB = os.environ.get("AQ_STAB", "0") == "1"
STAB_NF = int(os.environ.get("AQ_STAB_NF", "5"))
TAG = "comp_selection-tuning_" + TAG + (f"_stab{STAB_NF}" if STAB else "") + "_zgrid"
...
def stepwise(trm_np, kmax, seed=0):
    ...
    sfold = np.random.default_rng(seed + 7).integers(0, STAB_NF, gid.max() + 1)[gid] if STAB else None
    cur_auc = 0.5
    ztrace = []                    # per step: (step, z_full of the pick, cal, p_free) — training rows only
    ...
    for k in range(kmax):
        ...
        if GROUP:
            ...
            r_full, v_full = wm_t * (yt - pr), wm_t * pr * (1 - pr)
            z = score_all(r_full, v_full, sel, X=Xcur)
            if STAB:
                # the same exact score test on each leave-one-inner-fold-out subsample of the TRAINING
                # rows (eta is the training-rows fit; only the scoring rows change), then the
                # candidate that is strongest in its WORST subsample
                zmin = None
                for f in range(STAB_NF):
                    sub = torch.from_numpy((trm_np & (sfold != f)).astype(np.float32)).to(DEV)
                    zf = score_all(r_full * sub, v_full * sub, sel, X=Xcur)
                    zmin = zf if zmin is None else torch.minimum(zmin, zf)
                pick = int(torch.argmax(zmin).item())
            else:
                pick = int(torch.argmax(z).item())
            cal = float((wm_t * wm_t * pr * (1 - pr)).sum() / (v_full.sum() + 1e-12))
            ztrace.append((len(sel) + 1, round(float(z[pick]), 2), round(cal, 4), max(1, p - len(sel))))
            sel.append(pick)
        del Xcur
        Amat = design(sel)
        beta = newton_on(Amat, wm_t, RL, ksel=[MET[c]["k"] for c in sel])
    ...
    stepwise.last_trace = ztrace
    return sel

def k_at(alpha, ztrace, kmax):
    """the prefix length the multiplicity-corrected stop would have kept: entries up to (not
    including) the first step whose statistic fails 2 ln(p_free/alpha) * cal. alpha=0: never stop."""
    if not alpha: return kmax
    for step, zpick, cal, p_free in ztrace:
        if zpick < 2.0 * float(np.log(p_free / alpha)) * cal:
            return step - 1
    return kmax
...
oof_grid = {a: np.zeros(n, np.float32) for a in ZGRID}
HEAD = 1.0 if 1.0 in ZGRID else ZGRID[0]
per_fold = []
...
for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    ...
    ztr = getattr(stepwise, "last_trace", [])
    Kin, inner_aucs = (KMAX, []) if NO_INNER else pick_k(order, trm, seed=1000 + kf)
    wm_t = torch.from_numpy((w * trm).astype(np.float32)).to(DEV)
    fold_rec = {"K_inner": Kin, "ztrace": ztr, "alphas": {}}
    for alpha in ZGRID:
        Ka = min(Kin, k_at(alpha, ztr, KMAX))
        sel, forced = force_fast(order[:Ka], trm)
        Amat = design(sel)
        beta = newton_on(Amat, wm_t, RL, ksel=[MET[c]["k"] for c in sel])
        v = (Amat @ torch.from_numpy(beta.astype(np.float32)).to(DEV)).cpu().numpy()
        oof_grid[alpha][~trm] = v[~trm]
        del Amat
        fauc = float(roc_auc_score(y[~trm], v[~trm]))
        fold_rec["alphas"][str(alpha)] = {"K": Ka, "n_forced": len(forced), "K_total": len(sel), "fold_auc": round(fauc, 4),
                                          "forced": [b for b, _ in forced]}
        log(f"   outer {kf+1}/{NOUTER} · alpha {alpha:g} · K {Ka} (+{len(forced)} forced: {','.join(b for b,_ in forced) or 'none'}) = {len(sel)} phasors · fold AUC {fauc:.4f}")
    if DEV == "mps": torch.mps.empty_cache()
    per_fold.append(fold_rec)
    # the headline oof is the lens's rule (alpha=1 if on the grid, else the first grid entry)
    oof_outer[~trm] = oof_grid[HEAD][~trm]
...
grid_out = {}
for alpha in ZGRID:
    _a = float(roc_auc_score(y[tested & ~vault], oof_grid[alpha][tested & ~vault]))
    _e = _within_era(oof_grid[alpha])
    _kt = [f["alphas"][str(alpha)]["K_total"] for f in per_fold]
    grid_out[str(alpha)] = {"nested_auc": round(_a, 4), "within_era_auc": round(_e, 4), "phasors_per_fold": _kt,
                            "mean_phasors": round(float(np.mean(_kt)), 2),
                            "per_fold_auc": [f["alphas"][str(alpha)]["fold_auc"] for f in per_fold]}
    np.save(f"{D_}/oof_nested_{TAG}_a{alpha:g}.npy", oof_grid[alpha])
    log(f"GRID alpha {alpha:g}{' (control, never stop)' if not alpha else ''}: NESTED {_a:.4f} · WITHIN-ERA {_e:.4f} · phasors/fold {_kt} (mean {np.mean(_kt):.1f}) · bank {p}")
...
if ABLATE:
    json.dump({"tag": TAG, "headline_alpha": HEAD, "grid": grid_out, "stab": STAB_NF if STAB else 0,
               "nested_auc": round(auc_nested, 4), "within_era_auc": round(auc_within_era, 4), "per_fold": per_fold,
...
raise SystemExit("comp_selection-tuning.py is an AQ_ABLATE=1 estimate only (grid of stops); the deploy pass is fit_nested.py's")
