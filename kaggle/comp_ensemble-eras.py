"""comp_ensemble-eras.py — COMPETITION COPY of fit_nested.py (lens "ensemble-eras").

ERA-SPECIALISED ENSEMBLE. Inside every outer fold THREE lean models are fitted, one per century of
the husband's birth (pre-1800 pooled · 1800s · 1900s; edges AQ_EE_EDGES, default "1800,1900"), each
by the SAME procedure as the single model (stepwise to KMAX by the exact orthogonalised score test,
fast bodies forced, closed-form refit) but on the training rows of its own century only. Each test
couple is scored by the model of its own century. Three OOF score vectors come out of the same
folds, so the comparison is exact:
  single  — the standing procedure, one model on all training rows (the baseline)
  refit   — the single model's SELECTED STRUCTURE, weights re-fitted per century (drift of the MAP)
  ens     — structure AND weights per century (drift of WHICH angles matter, too)
Pooled nested AUC and within-era AUC are printed for all three; the headline NESTED / WITHIN-ERA
lines are the ensemble's. The pooled number of an era-routed model is confounded by its per-era
intercept (a step function of the birth year — the calendar); the within-era AUC is the clean
comparison, and it is the one that would show the angle-to-outcome map drifting with era.
Routing uses only the husband's birth year — a calendar quantity the chart already encodes through
the slow planets, never a label- or depth-derived quantity. Every selection, force and refit stays
inside the training rows of the outer fold.
...
TAG = "comp_ensemble-eras_" + TAG
EE_EDGES = tuple(int(x) for x in os.environ.get("AQ_EE_EDGES", "1800,1900").split(",") if x)
TAG += "_ee" + "-".join(map(str, EE_EDGES))
...
# ERA BLOCKS for routing: century of the husband's birth (pre-1800 pooled because 1500-1799 is
# only ~16k couples). A row with no parseable year goes to the last block (never happens here).
_yr_ee = pd.to_numeric(full.dob_a.astype(str).str.slice(0, 4), errors="coerce").to_numpy()
era_blk = np.digitize(np.nan_to_num(_yr_ee, nan=9999.0), EE_EDGES)
NERA = len(EE_EDGES) + 1
log("ERA BLOCKS (husband birth year): " + " · ".join(
    f"blk{e} [{int(np.nanmin(_yr_ee[era_blk==e]))}-{int(np.nanmax(_yr_ee[era_blk==e]))}] n={int((era_blk==e).sum()):,} y={float(y[era_blk==e].mean()):.3f}"
    for e in range(NERA)))
...
# ---- the nested estimate ----------------------------------------------------------------------
oof_outer = np.zeros(n, np.float32)       # ens  (era structure + era weights)  — the headline
oof_single = np.zeros(n, np.float32)      # single (the standing procedure, same folds)
oof_refit = np.zeros(n, np.float32)       # single structure, weights refit per era
per_fold = []
era_models = []                            # per fold, per era: the chosen terms + weights (the drift data)
tested = np.zeros(n, bool)

def _fit_predict(sel, trm_np):
    wm_t = torch.from_numpy((w * trm_np).astype(np.float32)).to(DEV)
    Amat = design(sel)
    beta = newton_on(Amat, wm_t, RL, ksel=[MET[c]["k"] for c in sel])
    v = (Amat @ torch.from_numpy(beta.astype(np.float32)).to(DEV)).cpu().numpy()
...
def _terms(sel, beta):
    ...
    for t_i, c in enumerate(sel):
        a_, c_ = float(beta[2 * t_i]), float(beta[2 * t_i + 1])
        out.append({"label": MET[c]["label"], "c": int(c), "w_cos": round(a_, 4), "w_sin": round(c_, 4),
                    "amp": round(float(np.hypot(a_, c_)), 4),
                    "phase_deg": round(float(np.degrees(np.arctan2(c_, a_))), 1)})
    return out

for kf in ([1] if ERA_SPLIT else range(NOUTER)):
    trm = fold != kf
    tested |= ~trm
    # --- single (the standing procedure) --------------------------------------------------------
    ...
    v, beta_s = _fit_predict(sel, trm)
    oof_single[~trm] = v[~trm]
    fauc_s = float(roc_auc_score(y[~trm], v[~trm]))
    fm = {"K_inner": Kin, "n_forced": len(forced), "fold_auc_single": round(fauc_s, 4),
          "forced": [b for b, _ in forced], "single_terms": _terms(sel, beta_s), "eras": []}
    log(f"   outer {kf+1}/{NOUTER} · single: K_inner {Kin} (+{len(forced)} forced) · fold AUC {fauc_s:.4f}")
    # --- per era: refit (same structure) and ens (own structure) --------------------------------
    for e in range(NERA):
        trm_e = trm & (era_blk == e)
        tst_e = (~trm) & (era_blk == e)
        # refit: the pooled structure, weights from this era's training rows only
        v_r, beta_r = _fit_predict(sel, trm_e)
        oof_refit[tst_e] = v_r[tst_e]
        # ens: the whole procedure on this era's training rows only
        order_e = stepwise(trm_e, KMAX, seed=1000 + kf + 100 * (e + 1))
        Kin_e, _ = (KMAX, []) if NO_INNER else pick_k(order_e, trm_e, seed=1000 + kf + 100 * (e + 1))
        sel_e, forced_e = force_fast(order_e[:Kin_e], trm_e)
        v_e, beta_e = _fit_predict(sel_e, trm_e)
        oof_outer[tst_e] = v_e[tst_e]
        ok = 0 < y[tst_e].sum() < tst_e.sum()
        a_s = float(roc_auc_score(y[tst_e], v[tst_e])) if ok else float("nan")
        a_r = float(roc_auc_score(y[tst_e], v_r[tst_e])) if ok else float("nan")
        a_e = float(roc_auc_score(y[tst_e], v_e[tst_e])) if ok else float("nan")
        shared = len(set(sel_e) & set(sel))
        fm["eras"].append({"era": e, "n_train": int(trm_e.sum()), "n_test": int(tst_e.sum()),
                           "K_inner": Kin_e, "n_forced": len(forced_e), "forced": [b for b, _ in forced_e],
                           "auc_single": round(a_s, 4), "auc_refit": round(a_r, 4), "auc_ens": round(a_e, 4),
                           "n_shared_with_single": shared,
                           "refit_terms": _terms(sel, beta_r), "ens_terms": _terms(sel_e, beta_e)})
        log(f"      era blk{e} · train {trm_e.sum():,} test {tst_e.sum():,} · K {Kin_e} (+{len(forced_e)} forced) "
            f"· shared/single {shared}/{len(sel)} · in-era AUC single {a_s:.4f} refit {a_r:.4f} ens {a_e:.4f}")
        if DEV == "mps": torch.mps.empty_cache()
    fauc = float(roc_auc_score(y[~trm], oof_outer[~trm]))
    fauc_r = float(roc_auc_score(y[~trm], oof_refit[~trm]))
    fm["fold_auc"] = round(fauc, 4); fm["fold_auc_refit"] = round(fauc_r, 4)
    per_fold.append(fm)
    log(f"   outer {kf+1}/{NOUTER} · fold AUC single {fauc_s:.4f} · refit {fauc_r:.4f} · ens {fauc:.4f}")
...
def per_block(oof):
    ...
    for e in range(NERA):
        _r = _m & (era_blk == e)
        out[f"blk{e}"] = round(float(roc_auc_score(y[_r], oof[_r])), 4) if 0 < y[_r].sum() < _r.sum() else None
    return out
res = {}
for name, oof in (("single", oof_single), ("refit", oof_refit), ("ens", oof_outer)):
    res[name] = {"nested_auc": round(float(roc_auc_score(y[_m], oof[_m])), 4),
                 "within_era_auc": round(float(within_era(oof)), 4), "per_block_auc": per_block(oof)}
    np.save(f"{D_}/oof_nested_{TAG}_{name}.npy", oof)
    log(f"   [{name:6}] pooled nested {res[name]['nested_auc']:.4f} · within-era {res[name]['within_era_auc']:.4f} · per block {res[name]['per_block_auc']}")
# DRIFT SUMMARY (the finding, if any): for the terms the single model chose, the phase of the
# era-refit weight vector versus the pooled one, per era — the angle-to-outcome map's rotation.
drift = []
for fm in per_fold:
    pooled = {t["c"]: t for t in fm["single_terms"]}
    for er in fm["eras"]:
        d = []
        for t in er["refit_terms"]:
            p0 = pooled[t["c"]]
            if p0["amp"] > 0 and t["amp"] > 0:
                dp = (t["phase_deg"] - p0["phase_deg"] + 180) % 360 - 180
                d.append(abs(dp))
        drift.append({"era": er["era"], "mean_abs_phase_shift_deg": round(float(np.mean(d)), 1) if d else None,
                      "median_abs_phase_shift_deg": round(float(np.median(d)), 1) if d else None,
                      "n_terms": len(d), "n_shared_with_single": er["n_shared_with_single"]})
auc_nested = res["ens"]["nested_auc"]; auc_within_era = res["ens"]["within_era_auc"]
...
if ABLATE:
    json.dump({"tag": TAG, "nested_auc": auc_nested, "within_era_auc": auc_within_era,
               "era_edges": list(EE_EDGES), "era_sizes": [int((era_blk == e).sum()) for e in range(NERA)],
               "variants": res, "drift": drift, "per_fold": per_fold,
...
raise SystemExit("comp_ensemble-eras.py: run with AQ_ABLATE=1 — the deploy pass is not part of this experiment")
