...
PYEOF
$PY <<'PYEOF' 2>&1 | grep -viE "^\s*[0-9]+%|B/s" | tail -1
import time
...
for acct in artafather arash0ash ashranet ashraasn; do
  if [ -f "$HOME/.kaggle/kaggle.$acct.json" ]; then
    printf '\n  ── %s ──\n' "$acct"
    AQ_MODEL="$MODEL" AQ_TEST="$SRC/test.csv" AQ_COMP_TEST="$COMP/test.csv" \
      $PY "$REPO/kaggle/tournament.py" round "$acct" || true
  fi
done

step "8. the board as it stands"
AQ_MODEL="$MODEL" AQ_TEST="$SRC/test.csv" AQ_COMP_TEST="$COMP/test.csv" $PY "$REPO/kaggle/tournament.py" board || true
